...
The genome summary report is where combination rules are applied. For each drug or drug class, the engine extracts all individual rules that have been applied. It then searches the combination rules for this organism and evaluates the logic strings (e.g. "rule1 & rule2 | rule3") to determine if any apply. If they do, the engine removes the individual rules that are overridden by the combination rule, and adds the combination rule to the list of rules that should be assessed for this drug or drug class. The highest level of interpretation is then applied.
