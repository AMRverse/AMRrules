...
When given an AMRFinderPlus input file, and an organism, AMRrules will apply the most relevant organism-specific rule to each AMRFinderPlus AMR genotype call. Below is an abbreviated example of the interpreted output file generated by AMRrules. (More examples can be found in **TESTS TBD**).
...
In all cases, genotype calls with no rules have an evidence grade of ``very low`` for the purposes of the summary report.
