...
When given an AMRFinderPlus input file, and an organism, AMRrules will apply the most relevant organism-specific rule to each AMRFinderPlus hit. Below is an abbreviated example of the output ``_interpreted.tsv`` file generated by AMRrules:
...
When matching a hit with a rule, AMRrules will first select all rules matching the ``variation type``.

Next, AMRrules will filter all possible rules to check if the ``nodeID`` from the rule matches to ``Hierarchy node``. If not, it will check the parent nodes of ``Hierarchy node`` to look for a matching ``nodeID``.

If no matching ``nodeID`` is found, AMRrules will then check the ``nucleotide accession``, ``protein accession`` and ``HMM accession`` fields, in that order.
